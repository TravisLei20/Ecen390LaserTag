...
static queue_t zQueue[FILTER_FREQUENCY_COUNT][Z_QUEUE_SIZE];
...
static queue_t outputQueue[FILTER_FREQUENCY_COUNT][OUTPUT_QUEUE_SIZE];
...
double filter_firFilter() {
  ...
  result += FIR_COEFFICIENTS[FIR_B_SIZE - 1] * queue_readElementAt(&xQueue, 0);
...
double filter_computePower(uint16_t filterNumber, bool forceComputeFromScratch,
  ...
  if (forceComputeFromScratch) {
    ...
    newestPower[filterNumber] =
        queue_readElementAt(outputQueue[filterNumber], OUTPUT_QUEUE_SIZE - 1);
